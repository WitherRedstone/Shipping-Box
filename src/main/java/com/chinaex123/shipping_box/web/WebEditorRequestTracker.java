 ...
 *   <li>调用 create(String) 创建 CompletableFuture 并注册；</li>
 *   <li>通过网络包将请求发送到游戏服务端；</li>
 *   <li>服务端处理完成后，通过响应包返回结果；</li>
 *   <li>响应处理器调用 complete(String, Response) 完成 Future；</li>
...
public final class WebEditorRequestTracker {
     ...
     * <p>
     * Key: 请求 ID（UUID 字符串）；
     * Value: 对应的 CompletableFuture，用于等待响应。
     * <p>
     ...
     * 创建一个新的请求追踪条目。
     * <p>
     * 生成一个 CompletableFuture 并注册到待处理列表中，
     * 后续可以通过请求 ID 完成这个 Future。
     *
     * @param requestId 请求的唯一标识符（通常使用 UUID）
     * @return 刚创建的 CompletableFuture，调用方可以等待它完成
     */
     ...
     * 完成一个请求并返回响应。
     * <p>
     * 当服务端处理完请求后，调用此方法将结果传递给等待的 Web 服务器。
     * 注意：如果请求 ID 不存在（可能已超时或被取消），则忽略此次完成操作。
     *
